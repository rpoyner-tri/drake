...
template <template <typename, int...> typename LinearSolverType,
          typename DerivedA>
using _LinearSolver = LinearSolverType<Eigen::Matrix<
    std::conditional_t<
        internal::is_double_or_symbolic_v<typename DerivedA::Scalar>,
...
template <template <typename, int...> typename LinearSolverType,
          typename DerivedA>
internal::_LinearSolver<LinearSolverType, DerivedA> GetLinearSolver(
    const Eigen::MatrixBase<DerivedA>& A) {
  if constexpr (internal::is_double_or_symbolic_v<typename DerivedA::Scalar>) {
    const internal::_LinearSolver<LinearSolverType, DerivedA> linear_solver(A);
    return linear_solver;
  } else {
    const auto A_val = autoDiffToValueMatrix(A);
    const internal::_LinearSolver<LinearSolverType, DerivedA> linear_solver(
        A_val);
    return linear_solver;
  }
}
...
class LinearSolver {
 public:
  using SolverType = internal::_LinearSolver<LinearSolverType, DerivedA>;
  explicit LinearSolver(const Eigen::MatrixBase<DerivedA>& A)
      : linear_solver_{GetLinearSolver<LinearSolverType>(A)} {
    if constexpr (!internal::is_double_or_symbolic_v<
