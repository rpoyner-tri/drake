...
# (e.g. my_lib.so.x.y.z).
_DYLIB_MATCH = re.compile(r"(.*\.so)(\.\d+)*$|(.*\.dylib)$")

_JAVA_LAUNCHER_TEMPLATE = """#!/bin/bash
...
def match_dynamic_library(filepath):
    return re.match(_DYLIB_MATCH, filepath)
...
class State:
    def __init__(self):
        ...
        # Mapping used to (a) check for unique shared library names and (b)
        # provide a mapping from library name to paths for RPath fixes (where
        # (a) is essential).
        # Structure: Map[ basename (Str) => full_path ]
        self.libraries_to_fix_rpath = {}

        # These are binaries (or Python shared libraries) that require RPath
        # fixes (and thus depend on `libraries_to_fix_rpath`), but by
        # definition are not depended upon by other components, and thus need
        # not be unique.
        # Structure: List[ Tuple(basename, full_path) ]
        self.binaries_to_fix_rpath = []

        # Files that are not libraries, but may still require fixing.
        # Structure: List[Str]
        self.potential_binaries_to_fix_rpath = []

    def do_install(self, args, src, dst):
        ...
        if needs_install(args, src, dst):
            ...
            # TODO(eric.cousineau): Unclear how RPath-patched file can be
            # deemed "up-to-date" by comparison?
            print("-- Up-to-date: {}".format(dst_full))
            # No need to check patching.
            return
        basename = os.path.basename(dst)
        if match_dynamic_library(basename):  # It is a library.
            if dst.startswith("lib/python") and not basename.startswith("lib"):
                ...
                if basename in self.libraries_to_fix_rpath:
                    sys.stderr.write(
                        "Multiple installation rules found for {}."
                        .format(basename))
                    sys.exit(1)
                self.libraries_to_fix_rpath[basename] = dst_full
        elif may_be_binary(dst_full):  # May be an executable.
            self.potential_binaries_to_fix_rpath.append(dst_full)

    def do_create_java_launcher(self, args, filename, classpath, jvm_flags,
                                main_class):
        # In list-only mode, just display the filename, don't do any real work.
        if args.list:
            print(filename)
        ...
        with open(filename, 'w') as launcher_file:
            content = _JAVA_LAUNCHER_TEMPLATE.format(
                strclasspath, jvm_flags, main_class)
            launcher_file.write(content)
        os.chmod(filename, stat.S_IRUSR | stat.S_IRGRP | stat.S_IROTH
                 | stat.S_IXUSR | stat.S_IXGRP | stat.S_IXOTH)

    def find_binary_executables(self):
        ...
        """
        if not self.potential_binaries_to_fix_rpath:
            return
        ...
        file_output = check_output(
            ["file"] + self.potential_binaries_to_fix_rpath).decode("utf-8")
        ...
        for line in file_output.splitlines():
            ...
            if re_result is not None:
                ...
                self.binaries_to_fix_rpath.append((basename, dst_full))

    def fix_rpaths_and_strip(self, args):
        # Add binary executables to list of files to be fixed up:
        self.find_binary_executables()
        # Only fix files that are installed now.
        fix_items = itertools.chain(
            self.libraries_to_fix_rpath.items(), self.binaries_to_fix_rpath)
        for basename, dst_full in fix_items:
            ...
            if sys.platform == "darwin":
                ...
                # error: 'Not enough room for program headers, try linking with
                # -N'
                if args.strip:
                    check_call([args.strip_tool, dst_full])
                linux_fix_rpaths(args, dst_full)
...
def linux_fix_rpaths(args, dst_full):
    ...
    for line in file_output.splitlines():
        ...
        if ldd_result[1] == 'not found' \
           or ldd_result[1].startswith(args.prefix):
            re_result = match_dynamic_library(ldd_result[0])
...
def execute_install_actions(args, actions, state):
    ...
    def install(*pos_args):
        state.do_install(args, *pos_args)

    def create_java_launcher(*pos_args):
        state.do_create_java_launcher(args, *pos_args)
    myglobals = dict(
...
def main(argv):
    ...
    state = State()
    execute_install_actions(args, actions, state)
    # Libraries paths may need to be updated in libraries and executables.
    state.fix_rpaths_and_strip(args)
