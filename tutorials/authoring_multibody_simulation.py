...
#
# In a high-level sense, you express different components of your robot using `<link>` tags and connect them via `<joint>` tags. Each `<link>` has three major subtags, `<visual>`, `<collision>`, and `<inertial>`, for its visualization, planning/collision checking, and dynamics aspects. For `<visual>` and `<collision>`, you can either use primitives (box, sphere, cylinder, etc.) or meshes (.gltf or .obj) to represent the underlying geometry.
#
...
# ### Mesh file formats
#
# You can use a mesh file for any of your robot `<link>` entries. The file formats that Drake supports (and the nature of that support) is [documented here](https://drake.mit.edu/doxygen_cxx/group__geometry__file__formats.html). Generally, glTF (`.gltf`) is the preferred format in Drake for visualization. OBJ (`.obj`) is another well-supported alternative. If you have other file formats, [Meshlab](https://www.meshlab.net/), an open-source software, is a handy tool to convert common formats to a `.obj`.
...
#
# Double-check the inertial properties, especially if the dynamic behavior is the focus of the simulation. To visually assess the inertial properties, use the inertia visualization layer in `ModelVisualizer`. In the controls, under `Scene >> drake`, check the `inertia` checkbox.
#
# To automatically calculate inertias for a URDF or SDFormat model, try the [fix_inertia](https://drake.mit.edu/pydrake/pydrake.multibody.fix_inertia.html) tool.
#
