...
bool check_handle(size_t n, handle p,
    const std::function<std::string(size_t)>& not_gc_message_function) {
  if (p.is_none()) {
    return false;
  }
...
void check_and_make_link(handle p0, handle p1,
    const std::function<std::string(size_t)>& not_gc_message_function) {
  // We are not using template argument indexing at the call site. In this
  // case, calling `p0` argument 1 makes the most sense (argument 0 denotes the
  // return value).
  if (!check_handle(1, p0, not_gc_message_function)) {
...
void make_arbitrary_ref_cycle(
  ...
  // We are not using template argument indexing at the call site. In this
  // case, calling the peers arguments 1 and 2 makes the most sense (argument 0
  // denotes the return value).
  check_and_make_ref_cycle(1, p0, 2, p1, not_gc_error);
}
