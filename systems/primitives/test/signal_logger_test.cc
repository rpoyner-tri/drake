...
GTEST_TEST(TestSignalLogger, LinearSystemTest) {
  ...
  auto logger = builder.AddSystem<SignalLogger<double>>(1, 1000,
                                                        kLogPerContext);
  logger->set_name("logger");
  builder.Cascade(*plant, *logger);

  // Test the AddSignalLogger helper method, too.
  auto logger2 = LogOutput(plant->get_output_port(), &builder, 1000,
                           kLogPerContext);
...
GTEST_TEST(TestSignalLogger, SetPublishPeriod) {
  ...
  auto logger = LogOutput(system->get_output_port(), &builder, 1000,
                          kLogPerContext);
