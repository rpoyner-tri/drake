...
void CheckStatistics(
  ...
  auto logger = LogOutput(source->get_output_port(0), &builder, 1000,
                          kLogPerContext);
...
GTEST_TEST(RandomSourceTest, CorrelationTest) {
  ...
  const auto* logger1 = LogOutput(random1->get_output_port(0), &builder, 1000,
                                  kLogPerContext);

  const auto* random2 = builder.AddSystem<RandomSourced>(
      RandomDistribution::kGaussian, kSize, kSampleTime);
  const auto* logger2 = LogOutput(random2->get_output_port(0), &builder, 1000,
                                  kLogPerContext);
