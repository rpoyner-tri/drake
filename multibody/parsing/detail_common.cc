...
using drake::multibody::parsing::internal::CollisionFilterGroupResolver;
...
void CollectCollisionFilterGroup(
  ...
  resolver->AddGroup(group_name, bodies, model_instance);
  ...
  for (auto ignore_node = next_child_element(
    ...
    // among each other.
    resolver->AddPair(group_name, target_name, model_instance);
  }
}
