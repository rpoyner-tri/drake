...
GTEST_TEST(MultibodyPlantConfigFunctionsTest, ContactRepTest) {
  using Hcr = geometry::HydroelasticContactRepresentation;
  std::vector<std::pair<const char*, Hcr>> known_values{
    std::pair("triangle", Hcr::kTriangle),
    std::pair("polygon", Hcr::kPolygon),
  };
