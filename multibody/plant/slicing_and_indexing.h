...
// Make a new, possibly smaller square matrix from square matrix `M`, by
// selecting the rows and columns indexed by `indices`.
...
// Write a new, possibly smaller square matrix into `out` from square matrix
// `M`, by selecting the rows and columns indexed by `indices`. If `out` has
// enough capacity, no allocation will be performed.
// @pre indices.size() <= M.rows() (or M.cols() since M is a square matrix).
// @pre indices argument is valid according to DemandIndicesValid().
template <typename T>
...
// Make a new, possibly smaller square matrix from square matrix `M`, by
// excluding the rows and columns indexed by `indices`.
// @pre indices.size() <= M.rows() (or M.cols() since M is a square matrix).
// @pre indices argument is valid according to DemandIndicesValid().
template <typename T>
...
// Make a new, possibly smaller matrix from matrix `M`, by excluding the
// columns indexed by `indices`.
// @pre indices.size() <= M.cols().
// @pre indices argument is valid according to DemandIndicesValid().
template <typename T>
...
// smaller matrix from M, by excluding the columns indexed by `indices`.
...
// Make a new, possibly smaller vector from vector `v`, by selecting the rows
// indexed by `indices`.
...
// Write a new, possibly smaller vector into `out` from vector `v`, by
// selecting the rows indexed by `indices`. If `out` has enough capacity, no
...
// Make a new, possibly smaller vector from vector `v`, by excluding the rows
// indexed by `indices`.
// @pre indices.size() <= v.size().
// @pre indices argument is valid according to DemandIndicesValid().
template <typename T>
...
// Make a new, possibly larger vector of size `rows_out`, by copying rows
// of `v` to rows indexed by `indices`. New rows are filled with zeros.
